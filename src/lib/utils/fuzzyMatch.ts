...
export function calculateAddressSimilarity(address1: string, address2: string): number {
  ...
  const normalized1 = address1.toLowerCase().trim().replace(/[^a-z0-9\s]/g, " ").replace(/\s+/g, " ");
  const normalized2 = address2.toLowerCase().trim().replace(/[^a-z0-9\s]/g, " ").replace(/\s+/g, " ");
...
export function calculateIndianNameSimilarity(name1: string, name2: string): number {
  ...
  const firstNameSimilarity = calculateStringSimilarity(components1.firstName, components2.firstName);
